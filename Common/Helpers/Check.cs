...
namespace CommonLib.Helpers
{
    ...
    public static class Check
    {
        ...
        {
            Require<ArgumentNullException>(value != null, string.Format("参数不可为{0}null", paramName));
        }
        ...
        public static void NotNullOrEmpty(string value, string paramName)
        {
            ...
            Require<ArgumentException>(!string.IsNullOrEmpty(value), string.Format("参数不可为{0}null或空", paramName));
        }
        ...
        {
            Require<ArgumentException>(value != Guid.Empty, string.Format("参数不可为{0}空guid", paramName));
        }
        ...
        public static void NotNullOrEmpty<T>(IEnumerable<T> collection, string paramName)
        {
            ...
            Require<ArgumentException>(collection.Any(), string.Format("参数不可为{0}空集合", paramName));
        }
        ...
        public static void NotNullOrEmpty<T>(IDictionary<string, T> dictionary, string paramName)
        {
            ...
            Require<ArgumentException>(dictionary.Any(),string.Format("参数不可为null或空集合"));
        }
            ...
            where T : IComparable<T>
        {
            ...
            string format = canEqual ? "参数不可小于等于" : "参数不可小于";
        ...
        {
            bool flag = canEqual ? value.CompareTo(target) >= 0 : value.CompareTo(target) > 0;
            string format = canEqual ? "参数不可大于等于" : "参数不可大于";
            Require<ArgumentOutOfRangeException>(flag, string.Format(format, paramName, target));
        }
            ...
            bool endEqual = false) where T : IComparable<T>
        {
            ...
            string message = startEqual
                ...
                : string.Format("参数{0}在{1}和{2}之间，不包括等于", paramName, start, end, start);
            Require<ArgumentOutOfRangeException>(flag, message);

            flag = endEqual ? value.CompareTo(end) <= 0 : value.CompareTo(end) < 0;
            message = endEqual
                ? string.Format("参数{0}在{1}和{2}之间", paramName, start, end)
                : string.Format("参数{0}在{1}和{2}之间，不包括等于", paramName, start, end, end);
            Require<ArgumentOutOfRangeException>(flag, message);
        }
        ...
        public static void DirectoryExists(string directory, string paramName = null)
        {
            ...
            Require<DirectoryNotFoundException>(Directory.Exists(directory), string.Format("目录{0}是否存在", directory));
        }
