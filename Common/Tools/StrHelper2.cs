...
namespace CommonLib.Tools
{
    public partial class StrHelper
    {
        ...
        {
            int index = ConstHelper.ChinesePinYin.IndexOf(text, StringComparison.Ordinal);
            if (index < 0)
            {
                return string.Empty;
            }
                    ...
                    else
                    {
                        ...
                        switch (chrAsc)
                        {
                            ...
                            case -5659:
                                pyString += "Tian"; break;
                            default:
                                for (int i = (ConstHelper.SpellCode.Length - 1); i >= 0; i--)
                                {
                                    if (ConstHelper.SpellCode[i] <= chrAsc)
                                    {
                                        //判断汉字的拼音区编码是否在指定范围内
                                        pyString += ConstHelper.SpellLetter[j];//如果不超出范围则获取对应的拼音
                                        break;
                                    }
                                }
                                break;
