...
namespace CommonLib.Tools
{
    public static class CheckHelper
    {
        ...
        {
            Require<ArgumentNullException>(value != null, string.Format("{0}为null", paramName));
        }
        ...
        public static void NotNullOrEmpty(string value, string paramName)
        {
            ...
            Require<ArgumentException>(!string.IsNullOrEmpty(value), string.Format("{0}为null或空", paramName));
        }
        ...
        {
            Require<ArgumentException>(value != Guid.Empty, string.Format("{0}guid为空", paramName));
        }
        ...
        public static void NotNullOrEmpty<T>(IEnumerable<T> collection, string paramName)
        {
            ...
            Require<ArgumentException>(collection.Any(), string.Format("{0}为null或空集合", paramName));
        }
        ...
        public static void NotNullOrEmpty<T>(IDictionary<string, T> dictionary, string paramName)
        {
            ...
            Require<ArgumentException>(dictionary.Any(), string.Format("字典为null或空"));
        }
            ...
            where T : IComparable<T>
        {
            ...
            string format = canEqual ? "不小于" : "不小于等于";
        ...
        {
            bool flag = canEqual ? value.CompareTo(target) >= 0 : value.CompareTo(target) > 0;
            string format = canEqual ? "不大于" : "不大于等于";
            Require<ArgumentOutOfRangeException>(flag, string.Format(format, paramName, target));
        }
            ...
            bool endEqual = false) where T : IComparable<T>
        {
            ...
            string message = startEqual
                ? string.Format("不在范围内", start, end)
                : string.Format("不在范围内包括边界", paramName, start, end, start);
            Require<ArgumentOutOfRangeException>(flag, message);

            flag = endEqual ? value.CompareTo(end) <= 0 : value.CompareTo(end) < 0;
            message = endEqual
                ? string.Format("不在范围内", paramName, start, end)
                : string.Format("不在范围内包括边界", paramName, start, end, end);
            Require<ArgumentOutOfRangeException>(flag, message);
        }
        ...
        public static void DirectoryExists(string directory, string paramName = null)
        {
            ...
            Require<DirectoryNotFoundException>(Directory.Exists(directory), string.Format("路径不存在", directory));
        }
        ...
        public static void FileExists(string fileName, string paramName = null)
        {
            ...
            Require<FileNotFoundException>(File.Exists(fileName), string.Format("文件不存在", fileName));
        }
